...
if [ $# -ne 2 ]; then
    echo "Usage: $0 <aws region> <aws resource>"
...
case $aws_service in
    ec2)
        ...
        --query 'Reservations[*].Instances[*].[InstanceId,Tags[?Key==`Name`].Value | [0],NetworkInterfaces[*].[PrivateIpAddress,NetworkInterfaceId,SubnetId]]' \
        --output json | jq -r '
            .[][] | {
                InstanceId: .[0],
                Name: .[1],
                IPAddresses: [
                    .[2][] | { PrivateIpAddress: .[0], NetworkInterfaceId: .[1], SubnetId: .[2] }
                ]
            } | "\(.InstanceId) \(.Name) \(.IPAddresses | map("\(.PrivateIpAddress) (\(.NetworkInterfaceId)) (\(.SubnetId))") | join(", "))"
        '         
        ;;
    rds)
